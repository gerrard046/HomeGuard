...
Pengujian HomeGuard dilaksanakan dalam tiga lapisan berjenjang yang menerapkan pendekatan \textit{Verification and Validation} (V\&V): (1) \textbf{pengujian unit} untuk memverifikasi kebenaran logika modul secara terisolasi; (2) \textbf{pengujian fungsional} menggunakan skrip layanan tiruan pada \textit{localhost} (CLI + \texttt{demo\_server.py}) untuk memvalidasi alur \textit{end-to-end} deteksi dan pemetaan OWASP; serta (3) \textbf{pengujian lingkungan lab virtual} menggunakan topologi jaringan yang lebih realistis (VirtualBox, subnet \texttt{192.168.56.0/24}) untuk mengukur akurasi deteksi dan kinerja operasional secara statistik.
...
Gambar~\ref{fig:pytest} menampilkan keluaran \texttt{pytest -v} yang mengonfirmasi seluruh 36 kasus uji lulus dalam waktu 1,25 detik.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{screenshots/02_pytest_36passed.png}
\caption{Keluaran \texttt{pytest -v --tb=no} menampilkan 36 kasus uji dari 6 modul seluruhnya lulus (\textit{36 passed in 1.25s}).}
\label{fig:pytest}
\end{figure}

\subsection{Pengujian Fungsional --- CLI dan Skrip Layanan Tiruan}

Lapisan kedua pengujian memvalidasi alur \textit{end-to-end} deteksi secara terkendali menggunakan skrip \texttt{demo\_server.py} yang menyalakan tiga layanan IoT tiruan pada \texttt{127.0.0.1} secara simultan: Telnet pada port 2323, HTTP dengan banner komponen usang (\texttt{lighttpd/1.4.35}) pada port 8080, dan RTSP pada port 8554. Skenario ini merepresentasikan profil port yang umum dijumpai pada kamera IP dan perangkat IoT dengan antarmuka web. Selanjutnya, CLI HomeGuard dijalankan dengan perintah \texttt{python3 cli.py --host 127.0.0.1 --ports 2323,8080,8554}. Tujuan pengujian lapisan ini adalah memverifikasi kebenaran: (a) identifikasi layanan dari \textit{banner}; (b) pemetaan ke kategori OWASP IoT Top 10; (c) mekanisme eskalasi \textit{severity} akibat komponen usang; dan (d) perhitungan skor risiko host --- pada kondisi yang sepenuhnya deterministik dan dapat diulang tanpa bergantung pada topologi jaringan.

Tabel~\ref{tab:cli_test} merangkum perbandingan antara hasil yang diharapkan dan hasil yang sebenarnya dideteksi oleh HomeGuard.
...
\hline
\textbf{Port} & \textbf{Layanan} & \textbf{Diharapkan} & \textbf{Terdeteksi} & \textbf{Kategori OWASP} \\
\hline
...
\hline
\multicolumn{5}{|l|}{$^\dagger$ Eskalasi dari TINGGI karena komponen usang \texttt{lighttpd/1.4.35} terdeteksi (+I5).} \\
\hline
\end{tabularx}
\end{table}

Ketiga layanan terdeteksi dengan benar (\textit{3/3 match}). Skor risiko host dihitung otomatis sebagai $S = \min(100,\, 90 + 3 \times 3) = 99$, sesuai Persamaan~\ref{eq:skor}. Gambar~\ref{fig:cli_output} menampilkan keluaran CLI lengkap dari skenario ini, termasuk rekomendasi mitigasi per temuan yang dihasilkan secara otomatis oleh HomeGuard.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{screenshots/01_cli_output.png}
\caption{Keluaran CLI HomeGuard saat memindai tiga layanan IoT tiruan pada \texttt{127.0.0.1}. Dua temuan KRITIS (Telnet port 2323 dan HTTP usang port 8080) serta satu temuan TINGGI (RTSP port 8554) berhasil teridentifikasi dengan rekomendasi mitigasi masing-masing.}
...
\hline
\textbf{Perangkat} & \textbf{TP} & \textbf{FP} & \textbf{FN} & \textbf{P} & \textbf{R} & \textbf{F1} \\
\hline
CCTV IP Camera   & 4 & 1 & 0 & 0,80 & 1,00 & 0,8889 \\
\hline
Wireless Router  & 4 & 0 & 0 & 1,00 & 1,00 & 1,0000 \\
\hline
Smart TV         & 3 & 1 & 1 & 0,75 & 0,75 & 0,7500 \\
\hline
Network Printer  & 5 & 1 & 0 & 0,83 & 1,00 & 0,9091 \\
\hline
Smart Plug       & 2 & 1 & 0 & 0,67 & 1,00 & 0,8000 \\
\hline
\textbf{Total}   & \textbf{18} & \textbf{4} & \textbf{1} & \textbf{0,82} & \textbf{0,95} & \textbf{0,8780} \\
\hline
\end{tabular}
\end{table}
...
\hline
\textit{Discovery} rata-rata (detik)     & 17,64 & 17,63 \\
\hline
\textit{Scan+Analisis} rata-rata (detik) & 6,13  & 6,13  \\
\hline
Total rata-rata (detik)                  & 23,77 & 23,76 \\
\hline
...
\hline
\textit{Cold start} / warm-up (detik)   & 99,77 & 98,98 \\
\hline
\end{tabular}
\end{table}

Waktu pemindaian bersifat sangat stabil ($\sigma \leq 0{,}04$ detik) dengan rata-rata 3,96 detik per host. Dari total waktu tersebut, fase \textit{network discovery} menyumbang $\approx$17,63 detik (mengirim TCP ping ke seluruh subnet) dan fase \textit{scan+analisis} $\approx$6,13 detik. Perbedaan signifikan antara \textit{cold start} ($\approx$99 detik) dan pemindaian berikutnya ($\approx$23,77 detik) disebabkan oleh pengunduhan data CVE dari NVD API pada percobaan pertama (warm-up); data kemudian di-\textit{cache} sehingga fase \textit{scan+analisis} turun dari $\approx$81 detik menjadi $\approx$6 detik pada percobaan berikutnya.

\subsubsection{Konsumsi Sumber Daya}

Pemantauan sumber daya selama pemindaian dilaksanakan dalam dua sesi independen. Tabel~\ref{tab:resource} merangkum hasil pengukuran per sesi (trial pertama setiap sesi merupakan \textit{cold start} dan tidak dihitung ke rata-rata operasional).

\begin{table}[htbp]
\caption{Konsumsi Sumber Daya Selama Pemindaian (6 Host)}
\label{tab:resource}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metrik} & \textbf{Sesi 1 ($n$=3)$^\dagger$} & \textbf{Sesi 2 ($n$=4)} \\
\hline
CPU rata-rata (\%) & 4,4 & 4,9 \\
\hline
CPU puncak (\%)    & 25,0 & 25,0 \\
\hline
RAM rata-rata (MB) & 36,7 & 37,0 \\
\hline
RAM puncak (MB)    & 37,0 & 37,3 \\
\hline
\textit{Cold start} CPU avg (\%) & 1,3 & 1,2 \\
\hline
\textit{Cold start} RAM avg (MB) & 34,7 & 34,8 \\
\hline
\multicolumn{3}{|l|}{$^\dagger$ 1 percobaan dikecualikan (CPU maks 2605,9\% --- artefak pengukuran).} \\
\hline
\end{tabular}
\end{table}

HomeGuard beroperasi dengan jejak yang sangat ringan: rata-rata CPU 4,4--4,9\% (puncak 25,0\%) dan RAM 36,7--37,0 MB (puncak 37,3 MB) pada sistem dengan 12 \textit{core} dan 23.886 MB RAM. Perbedaan kecil antar sesi mengonfirmasi konsistensi hasil. Profil ini membuktikan bahwa aplikasi dapat berjalan di komputer rumahan dengan spesifikasi minimal.

% =========================================================
\section{Pembahasan}
% =========================================================

Hasil pengujian tiga lapisan menunjukkan bahwa HomeGuard mampu memenuhi tujuan rancangannya secara konsisten. Pada lapisan fungsional (CLI + \texttt{demo\_server.py}), seluruh tiga layanan tiruan --- Telnet, HTTP usang, dan RTSP --- terdeteksi dengan pemetaan OWASP yang tepat (\textit{3/3 match}), mekanisme eskalasi \textit{severity} akibat komponen usang berfungsi sesuai rancangan, dan skor risiko host dihitung dengan benar (skor 99). Pengujian ini memverifikasi kebenaran logika inti deteksi pada kondisi yang terkontrol sepenuhnya. Pada lapisan lingkungan lab virtual (5 VM, subnet \texttt{192.168.56.0/24}), aplikasi mencapai F1-\textit{score} deteksi port sebesar 0,8780 dengan \textit{recall} 0,9474 --- artinya hampir seluruh port berisiko pada topologi jaringan yang lebih realistis berhasil teridentifikasi. Pendekatan \textit{rule-based} berbasis OWASP IoT Top 10 yang dipadukan dengan skoring heuristik memungkinkan prioritisasi risiko yang mudah dipahami pengguna non-teknis. Konsisten dengan temuan Neshenko et al.\ \cite{neshenko2019} bahwa mayoritas eksploitasi IoT menargetkan lapisan jaringan, kategori yang paling sering muncul pada kedua skenario pengujian adalah I2 (\textit{Insecure Network Services}), diikuti I7 (\textit{Insecure Data Transfer and Storage}).

Dari sisi kinerja, waktu pemindaian rata-rata 3,96 detik per host dengan standar deviasi yang sangat kecil ($\sigma \leq 0{,}04$ detik), mencerminkan konsistensi yang tinggi. Fase \textit{discovery} mendominasi waktu total ($\approx$17,63 detik) karena mengirim TCP ping ke seluruh /24 subnet, sedangkan fase \textit{scan+analisis} hanya $\approx$6,13 detik. Lonjakan waktu pada \textit{cold start} ($\approx$99 detik) disebabkan oleh pengunduhan data CVE dari NVD API, bukan oleh proses pemindaian itu sendiri. Konsumsi sumber daya yang rendah (CPU rata-rata 4,4--4,9\%, puncak 25,0\%; RAM rata-rata 36,7--37,0 MB, puncak 37,3 MB) mengonfirmasi bahwa aplikasi dapat berjalan pada komputer rumahan tanpa beban yang berarti.
...
Penelitian ini berhasil merancang dan mengimplementasikan HomeGuard, sebuah aplikasi pemindai keamanan jaringan rumah berbasis Python yang mengintegrasikan modul \textit{network discovery}, \textit{port scanning}, \textit{service detection}, klasifikasi perangkat, pemeriksaan TLS/HTTPS, pemeriksaan kredensial bawaan (\textit{opt-in}), dan pemetaan kerentanan berbasis OWASP IoT Top 10. Inti pemindaian diimplementasikan hanya dengan pustaka standar Python tanpa memerlukan hak akses \textit{root}, sehingga portabel dan mudah digunakan oleh pengguna rumahan. Pengujian dilaksanakan dalam tiga lapisan: (1) \textbf{pengujian unit} (36/36 lulus, \texttt{pytest}) memverifikasi kebenaran logika modul secara terisolasi; (2) \textbf{pengujian fungsional} menggunakan CLI dan skrip layanan tiruan (\texttt{demo\_server.py}) pada \texttt{127.0.0.1} memvalidasi alur \textit{end-to-end} --- ketiga layanan (Telnet, HTTP usang, RTSP) terdeteksi dengan pemetaan OWASP yang benar (\textit{3/3 match}) dan skor host dihitung tepat (99/100); (3) \textbf{pengujian lingkungan lab virtual} (5 VM, subnet \texttt{192.168.56.0/24}) menghasilkan F1-\textit{score} deteksi port 0,8780 (\textit{precision} 0,8182, \textit{recall} 0,9474) dengan waktu pemindaian rata-rata 3,96 detik per host ($\sigma \leq 0{,}04$ detik, Discovery $\approx$17,63s + Scan+Analisis $\approx$6,13s) dan konsumsi sumber daya yang ringan (CPU rata-rata 4,4--4,9\%, puncak 25,0\%; RAM rata-rata 36,7--37,0 MB, puncak 37,3 MB). Dengan demikian, aplikasi mampu mengidentifikasi kerentanan perangkat IoT dan memberikan rekomendasi mitigasi yang \textit{actionable}, didukung pelaporan dalam format JSON dan PDF serta otomasi pengujian melalui \textit{Continuous Integration} (GitHub Actions).
