...
\begin{enumerate}
    ...
    \item Mengevaluasi efektivitas aplikasi yang dikembangkan dalam mendeteksi kerentanan keamanan pada perangkat IoT melalui serangkaian pengujian pada lingkungan jaringan rumah yang sebenarnya (\textit{real-world testing}).
\end{enumerate}
...
\begin{enumerate}
    \item Pengujian dilakukan secara eksklusif pada jaringan Wi-Fi rumah milik peneliti sendiri, sesuai dengan prinsip etika keamanan siber dan ketentuan hukum yang berlaku.
    ...
    \item Pengujian dunia nyata dibatasi pada 8 unit perangkat IoT yang tersedia di lingkungan rumah peneliti (lihat Tabel\ \ref{tab:device_list}).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Pengujian.} Pengujian unit (\texttt{pytest}, 36 kasus uji) dan pengujian fungsional pada jaringan Wi-Fi rumah milik peneliti.
...
Pengujian dilaksanakan pada jaringan Wi-Fi domestik dengan delapan unit perangkat IoT sebagaimana dirinci pada Tabel\ \ref{tab:device_list}.

\begin{table}[htbp]
\caption{Daftar Perangkat IoT pada Lingkungan Pengujian}
\label{tab:device_list}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\columnwidth}{|c|X|l|}
\hline
\textbf{No} & \textbf{Jenis Perangkat} & \textbf{Vendor} \\
\hline
1 & Kamera IP (CCTV) & Hikvision \\
\hline
2 & Smart TV & Samsung \\
\hline
3 & Smart Plug & TP-Link (Kasa) \\
\hline
4 & Router Wi-Fi (AP utama) & TP-Link \\
\hline
5 & Single-board computer & Raspberry Pi Foundation \\
\hline
6 & Smart Speaker & Google (Nest) \\
\hline
7 & Printer jaringan & Hewlett-Packard (HP) \\
\hline
8 & Laptop pengguna (kontrol) & Intel (NIC) \\
\hline
\end{tabularx}
\end{table}
...
Pengujian dunia nyata dilakukan pada jaringan Wi-Fi rumah peneliti terhadap delapan perangkat IoT. Tabel\ \ref{tab:hasil_scan} menyajikan hasil lengkap pemindaian yang diurutkan menurut skor risiko.

\begin{table}[htbp]
\caption{Hasil Pemindaian Jaringan Rumah (Lengkap)}
\label{tab:hasil_scan}
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\columnwidth}{|X|c|c|c|}
\hline
\textbf{Perangkat} & \textbf{OWASP} & \textbf{Severity} & \textbf{Skor} \\
\hline
Kamera IP (Hikvision) & I1,I2,I3,I7 & KRITIS & 96 \\
\hline
Router Wi-Fi (TP-Link) & I1,I2,I7,I9 & KRITIS & 90 \\
\hline
Smart TV (Samsung) & I3,I7 & TINGGI & 78 \\
\hline
Printer (HP) & I2,I3,I7 & TINGGI & 75 \\
\hline
Smart Plug (TP-Link) & I2 & SEDANG & 53 \\
\hline
Smart Speaker (Google) & I2 & SEDANG & 50 \\
\hline
SBC (Raspberry Pi) & I2 & SEDANG & 50 \\
\hline
Laptop pengguna & --- & AMAN & 0 \\
\hline
\end{tabularx}
\end{table}

Dari delapan perangkat yang diuji, tujuh perangkat (87,5\%) terdeteksi memiliki minimal satu temuan kerentanan jaringan, sementara satu perangkat (laptop kontrol pengguna) dinilai AMAN. Dua perangkat --- kamera IP dan router Wi-Fi --- terklasifikasi KRITIS akibat layanan Telnet yang masih aktif dengan kredensial bawaan pabrik, menggemakan pola eksploitasi yang sama dengan kasus Mirai Botnet\ \cite{antonakakis2017}.

% =========================================================
\section{Pembahasan}
% =========================================================

Hasil pengujian menunjukkan bahwa HomeGuard mampu menjalankan keseluruhan \textit{pipeline} penemuan host hingga pemetaan kerentanan secara otomatis dan menghasilkan laporan yang \textit{actionable}, lengkap dengan rekomendasi mitigasi per temuan. Pendekatan \textit{rule-based} berbasis OWASP IoT Top 10 yang dipadukan dengan skoring heuristik memungkinkan prioritisasi risiko yang mudah dipahami pengguna non-teknis. Konsisten dengan temuan Neshenko et al.\ \cite{neshenko2019} bahwa mayoritas eksploitasi IoT menargetkan lapisan jaringan, kategori yang paling sering muncul pada pengujian adalah I2 (\textit{Insecure Network Services}), diikuti I7 (\textit{Insecure Data Transfer and Storage}). Penambahan modul pemeriksaan TLS/HTTPS dan kredensial bawaan memperkuat cakupan deteksi pada kategori I7, I3, I1, dan I9 yang sebelumnya tidak terjangkau hanya oleh pemindaian port.

Distribusi tingkat keparahan pada pengujian jaringan rumah menunjukkan pola yang konsisten dengan laporan Bitdefender\ \cite{bitdefender2024}: perangkat lawas atau \textit{entry-level} (kamera IP, router bawaan ISP) cenderung memiliki kerentanan paling kritis, sedangkan perangkat dengan ekosistem tertutup yang dikelola produsen besar menunjukkan \textit{severity} yang relatif lebih rendah.

Keterbatasan utama prototipe ini meliputi: cakupan katalog port yang masih terbatas, basis data OUI berupa \textit{seed} (belum lengkap), deteksi komponen usang berbasis pola \textit{signature} statis yang belum terkorelasi dengan basis data CVE secara langsung, serta ukuran sampel pengujian dunia nyata (8 perangkat pada 1 jaringan rumah) yang masih terbatas untuk generalisasi statistik.
...
Penelitian ini berhasil merancang dan mengimplementasikan HomeGuard, sebuah aplikasi pemindai keamanan jaringan rumah berbasis Python yang mengintegrasikan modul \textit{network discovery}, \textit{port scanning}, \textit{service detection}, klasifikasi perangkat, pemeriksaan TLS/HTTPS, pemeriksaan kredensial bawaan (\textit{opt-in}), dan pemetaan kerentanan berbasis OWASP IoT Top 10. Inti pemindaian diimplementasikan hanya dengan pustaka standar Python tanpa memerlukan hak akses \textit{root}, sehingga portabel dan mudah digunakan oleh pengguna rumahan. Pengujian unit (36/36 lulus) dan pengujian fungsional terkontrol membuktikan kebenaran logika pemetaan kerentanan dan perhitungan skor risiko. Pengujian pada jaringan rumah nyata terhadap delapan perangkat IoT menunjukkan bahwa 87,5\% perangkat memiliki minimal satu temuan kerentanan, dengan kategori I2 sebagai temuan paling dominan. Dengan demikian, aplikasi mampu mengidentifikasi kerentanan perangkat IoT dan memberikan rekomendasi mitigasi yang \textit{actionable}, didukung pelaporan dalam format JSON dan PDF serta otomasi pengujian melalui \textit{Continuous Integration} (GitHub Actions).

\subsection{Saran}

Pengembangan lanjutan yang disarankan meliputi: (1) korelasi versi komponen dengan basis data CVE (kategori I5) melalui NVD API agar deteksi komponen usang lebih presisi; (2) pemutakhiran basis data OUI IEEE secara lengkap untuk meningkatkan akurasi identifikasi vendor; (3) perluasan pengujian dunia nyata pada lebih banyak jaringan rumah guna memperkuat generalisasi statistik; (4) penambahan dukungan deteksi protokol IoT non-TCP (mis. Zigbee, Z-Wave, BLE); dan (5) pengayaan basis aturan klasifikasi perangkat dengan teknik \textit{machine learning} ringan untuk meningkatkan akurasi \textit{fingerprinting}.
