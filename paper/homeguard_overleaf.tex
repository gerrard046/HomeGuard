...
keamanan siber di tingkat rumah tangga. Seluruh kode sumber aplikasi tersedia
secara terbuka di \url{https://github.com/gerrard046/HomeGuard}.
\end{abstract}
...
modul inti, yaitu pemetaan kerentanan (\texttt{vulnmap}), penemuan host
(\texttt{discovery}), klasifikasi perangkat (\texttt{classify}),
pemindaian port (\texttt{portscan}), orkestrasi pemindaian
(\texttt{scanner}), dan pemeriksaan TLS (\texttt{tlscheck}), dengan total
36 kasus uji. Seluruh kasus uji \textbf{lulus} (36/36); kasus-kasus uji
yang representatif dirangkum pada Tabel~\ref{tab:unit}. Pengujian ini
memvalidasi antara lain kebenaran logika pemetaan OWASP, perhitungan
skor, normalisasi MAC, \textit{vendor lookup}, \textit{parsing} tabel
ARP, dan ekspansi CIDR.

\begin{table}[ht]
    \centering
    \caption{Ringkasan Kasus Uji Representatif (Total: 36/36 Lulus)}
    \label{tab:unit}
...
mitigasi yang \textit{actionable}. Seluruh kode sumber, kasus uji, dan
dokumentasi HomeGuard tersedia secara terbuka pada repositori GitHub:
\url{https://github.com/gerrard046/HomeGuard} \cite{b23}.
...
\bibitem{b23} R. H. Marbun, R. G. R. Ramadhan, and R. Kresensia, ``HomeGuard: Aplikasi Pemindai Keamanan Jaringan Rumah,'' GitHub repository, 2026. [Online]. Available: \url{https://github.com/gerrard046/HomeGuard}
\end{thebibliography}
